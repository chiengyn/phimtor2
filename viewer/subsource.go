...
// the SubtitleProvider interface so this service can offer it as an alternative
// source; the browser never talks to subsource directly (the API key is sent in
// the X-API-Key header and must not leak).
//
// BYTE-FOR-BYTE DUPLICATE of admin/subsource.go, by the same convention that
// duplicates models.go and static/mkvplayer.js. Keep the two in sync.
//
// This is the provider most exposed to the repo-wide charset gap noted in
// opensubtitles.go: extractSubtitleFromZip returns whatever bytes the archive
// held, with no encoding detection, so a non-UTF-8 .srt becomes mojibake.
//
