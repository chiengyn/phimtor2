...
// from the joined torrent_sources row; the raw .torrent bytes are never loaded.
// Mirrors admin's Video (the viewer reads what admin writes); the viewer never
// sends Magnet to the browser — it POSTs it to the streamer server-side.
type Video struct {
	ID         int64     `json:"id"`
