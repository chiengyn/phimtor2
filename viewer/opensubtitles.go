...
// browser never talks to OpenSubtitles directly: the Api-Key would leak and the
// API does not allow authenticated cross-origin calls, so every request is
// funneled through this server. Matching is by text query + season/episode only
// — there is no moviehash, since nothing here holds torrent data.
//
// This is a DUPLICATE of admin/opensubtitles.go, minus the pieces the viewer
// already owns (stripASSOverrides lives in server.go) or does not need
// (parseSubtitleQuery — the viewer seeds its query from the catalog row, never
// from a video file name). Keep the two in sync; see subtitleprovider.go.
//
// One thing differs operationally, not in code: the API key is the SAME shared
// account the admin uses, and here it is reachable by any signed-in visitor
// rather than one operator. That is what the limiters in subtitleService guard.
//
// Known gap, shared with admin: there is no charset detection anywhere. A
// Windows-1258/CP1252 Vietnamese .srt is stored and served as mojibake. The
// /download call below asks for WebVTT, which comes back UTF-8, so this path is
// largely unaffected — subsource ZIPs are the real exposure. Fix belongs in both
// services at once.
